...
function ProductAuthoringMain(productprops: ProductAuthoringMainProps) {
          ...
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            >
              {values.containedPackages?.map((containedPackage, index) =>
                     ...
                     (
                        <Tab
                            label={containedPackage?.packageDetails?.productName? containedPackage?.packageDetails?.productName?.pt.term:"untitled"}
                            {...a11yProps(index)}
                            key={index}
                        />
                    )
              ...
              <Tab
                  icon={<SearchIcon />}
                  onClick={ ()=>handlePackageCreation(values.containedPackages?.length ? values.containedPackages?.length+2 :0 ) }
                  {...a11yProps(values.containedPackages?.length ? values.containedPackages?.length+2 :0)}
                  key={values.containedPackages?.length ? values.containedPackages?.length+2 :0}
              />
            </Tabs>
          </Box>
          {values.containedPackages?.map((containedPackage, index) => (
  ...
  const ContainedProducts: FC<ContainedProductsProps> = ({
                      ...
                      >
                        <Grid xs={40} item={true}>
                          <Stack direction="row" spacing={2} alignItems="center">
                            <Grid item xs={10}>
                              <Typography>
                                {containedProduct.productDetails?.productName ? containedProduct.productDetails?.productName?.pt.term : "untitled"}
                              </Typography>
                            </Grid>
                            <Grid container justifyContent="flex-end">
                              <Stack direction="row" spacing={0} alignItems="center">
    ...
    return (
        <>
          {partOfPackage? (<Level2Box component="fieldset">
            <legend>Contained Products</legend>
            <ProductDetails></ProductDetails>

          </Level2Box>) : ( <Level1Box component="fieldset">
            <legend>Contained Products</legend>
            <ProductDetails></ProductDetails>
