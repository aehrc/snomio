...
function ExportTickets(props: Props) {
    ...
    return(<>
        ...
        <Button variant="contained" component="label" onClick={() => writeTickets()} disabled={isWorking}>
        { isWorking  && 
            <CircularProgress
                size={20}
                sx={{
        ...
        :
            <>
            { isFinished ? (<Alert variant="filled" severity='success'>{`Export Finished!`}</Alert>) :  <Box/> }
            </>
        }
    </>)
    }
