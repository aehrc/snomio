...
async function downloadAttachment(
  ...
  const ticketNumber =
    Math.ceil(parseInt(jiraTicket.split("-")[1]) / 10000) * 10000;
  ...
  if (fs.existsSync(filePath)) {
    ...
    return {
      ...
      thumbnailPath: fs.existsSync(thumbNailAlternativePath)
        ? thumbNailAlternativePath
        : null,
    };
  }
  return null;
...
export async function doExport(props: SaveRequest) {
  ...
  fs.writeFileSync(
    props.directory + "/" + props.filename,
    JSON.stringify(ticketsToSave, null, 4),
  );
...
function parseRelativeDate(dateStr: string): moment.Moment {
  ...
  date = moment(date.format("YYYY-MM-DD")).set({
    hour: hours,
    minute: minutes,
    second: 0,
    millisecond: 0,
...
async function createTicketDto(
  props: SaveRequest,
  issue: AmtJiraTicket,
): Promise<TicketDto> {
  ...
  for (let k = 0; k < issue.fields.comment?.total; k++) {
    const createdDateToConvert =
      issue.renderedFields.comment.comments[k].updated ||
      issue.renderedFields.comment.comments[k].created ||
      new Date().toDateString().split(" ").slice(1).join("-");
    let isoDate = convertDate(createdDateToConvert).format(
      "YYYY-MM-DDTHH:mm:ss.SSSZ",
    );
    if (isoDate == null || isoDate.toLocaleLowerCase() === "invalid date") {
      isoDate = parseRelativeDate(createdDateToConvert).format(
        "YYYY-MM-DDTHH:mm:ss.SSSZ",
      );
    }
